...
- **Docker MCP image smoke test polls for readiness** — the PR-only `Test MCP image` job waited a fixed 10s before curling `/health`, which the server routinely exceeds, so the check failed on every pull request while real release pushes succeeded. It now polls for up to 60s and dumps container logs on timeout.
