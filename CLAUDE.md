...
- **API mode** (if API key set): Anthropic, Google Gemini, OpenAI, Moonshot/Kimi, MiniMax — detected in registry order; `SKILL_SEEKER_PROVIDER` forces one. Models: `SKILL_SEEKER_MODEL` (global) or `ANTHROPIC_MODEL`/`GOOGLE_MODEL`/`OPENAI_MODEL`/`MOONSHOT_MODEL`/`MINIMAX_MODEL`; `ANTHROPIC_BASE_URL` for compatible endpoints. MiniMax adds `MINIMAX_API_REGION` (`global_en`/`cn_zh`), `MINIMAX_API_PROTOCOL` (`openai`/`anthropic`) and `MINIMAX_THINKING` (`adaptive`/`disabled`, OpenAI protocol only; validated at `AgentClient` construction). Video input (`AgentClient.call_with_video()`, MiniMax-M3 only, 50 MB cap) is registry-driven like images: `supports_video`, `video_models`, `video_max_bytes`, `thinking_modes`, `thinking_env` on the provider entry. Vision OCR provider: `SKILL_SEEKER_VISION_PROVIDER` (`auto` picks the first image-capable provider with a key).
