...
class TestDocScraperEntrypoint(unittest.TestCase):
    ...
    def test_run_scraping_uses_scrape_all(self):
        ...
        class DummyConverter:
            ...
            def checkpoint_exists(self): return False
            def load_checkpoint(self): pass
            def clear_checkpoint(self): pass
            def scrape_all(self): self.scrape_all_called = True
            def build_skill(self): self.build_skill_called = True
            def save_checkpoint(self): self.save_checkpoint_called = True
    ...
    def test_run_scraping_keyboard_interrupt(self):
        ...
        class DummyConverter:
            ...
            def load_checkpoint(self): pass
            def clear_checkpoint(self): pass
            def scrape_all(self): raise KeyboardInterrupt
            def build_skill(self): self.build_skill_called = True
            def save_checkpoint(self): self.save_checkpoint_called = True
        converter = DummyConverter()
        with patch("skill_seekers.cli.doc_scraper.DocToSkillConverter", lambda *_a, **_kw: converter):
