...
### Workflow 5: Atlas Cloud

[Atlas Cloud](https://www.atlascloud.ai/?utm_source=github&utm_medium=link&utm_campaign=skill_seekers) is a full-modal, OpenAI-compatible AI inference platform. Its chat models are reached through the same OpenAI-compatible adaptor used by MiniMax/Kimi/DeepSeek/Qwen.
...
# 2. Enhance with the default model (deepseek-ai/deepseek-v4-pro)
skill-seekers enhance output/react/ --target atlas

# 2b. Or pick any chat model from the list below
skill-seekers enhance output/react/ --target atlas --model zai-org/glm-5

# 3. Package for Atlas
skill-seekers package output/react/ --target atlas

# 4. Upload to Atlas (validates with API)
skill-seekers upload react-atlas.zip --target atlas

# Access at: https://www.atlascloud.ai/
```

> `deepseek-ai/deepseek-v4-pro` is a reasoning model — give it enough `max_tokens`
> (>= 512) so the chain-of-thought does not consume the whole budget and leave an
> empty completion.

<details>
<summary>All Atlas Cloud chat models (59)</summary>

Synced with the official list at <https://www.atlascloud.ai/models>. Any of these
can be passed to `--model` with `--target atlas`:
...
### Workflow 6: Export to All Platforms
...
- OpenAI: GPT-4o
- MiniMax: MiniMax-M3
- Atlas Cloud: deepseek-ai/deepseek-v4-pro (any of its 59 chat models via `--model`)
...
`openai`, `minimax`, `atlas`, `kimi`, `deepseek`, `qwen`, `openrouter`, `together`,
