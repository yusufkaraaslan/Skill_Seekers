...
Any of the chat models below can be selected with `--model` when enhancing:

```bash
export ATLAS_API_KEY=<your-atlas-api-key>
skill-seekers enhance output/react/ --target atlas --model deepseek-ai/deepseek-v4-pro
```

`deepseek-ai/deepseek-v4-pro` is the default. It is a reasoning model, so give it enough room (`max_tokens >= 512`) to avoid empty completions.

<details>
<summary>All Atlas Cloud chat models (59)</summary>

Synced with the official list at <https://www.atlascloud.ai/models>:
