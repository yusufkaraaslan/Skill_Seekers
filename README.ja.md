...
[Atlas Cloud](https://www.atlascloud.ai/?utm_source=github&utm_medium=link&utm_campaign=skill_seekers) — フルモーダルかつ OpenAI 互換の AI 推論プラットフォームです。Skill Seekers は `ATLAS_API_KEY` と `--target atlas` によるパッケージング／エンハンス先としてサポートしています。
...
**前提条件:** Python 3.10+、Git。はじめての方は → **[Bulletproof Quick Start](BULLETPROOF_QUICKSTART.md)** 🎯
